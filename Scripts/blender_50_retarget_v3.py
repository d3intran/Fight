...
K_MAT = {}
for t in MAP:
    if S_FRAME_REST[t] is not None and T_FRAME_REST[t] is not None:
        K_MAT[t] = Q @ S_FRAME_REST[t] @ T_FRAME_REST[t].inverted()
print("逐骨骼静止基准差（K 的旋转角，度）:")
for t in sorted(K_MAT):
    ang = math.degrees(K_MAT[t].to_quaternion().angle)
    flag = "" if ang < 8 else ("  <- 基准差大，已补偿" if ang < 60 else "  <- 基准差极大，已补偿")
    print(f"   {t:12s} {ang:7.1f}°{flag}")
