    ...
    if "BP_DariusCharacter" in a.get_name():
        for c in a.get_components_by_class(unreal.StaticMeshComponent):
            if "Axe" in c.get_name() or "Weapon" in c.get_name():
                print("Found weapon comp in PIE:", c.get_name())
                print("Before WorldScale:", c.get_world_scale())
                c.set_editor_property("absolute_scale", True)
                c.set_editor_property("relative_scale3d", unreal.Vector(1.0, 1.0, 1.0))
                print("After WorldScale:", c.get_world_scale())
