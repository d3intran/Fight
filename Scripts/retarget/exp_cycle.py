...
用法：
    uv run --no-project python Scripts/retarget/exp_cycle.py            # 跑默认实验组
    uv run --no-project python Scripts/retarget/exp_cycle.py e2 e3      # 只跑指定 tag
"""
import json
import os
import re
...
def run_script(script, log):
    ...
    t0 = time.time()
    cmd = ["uv", "run", "--no-project", "python"] + GW + [ROOT + "/Scripts/retarget/" + script]
    try:
